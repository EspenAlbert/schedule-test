# autogen_fast/projectapi/TestAccProjectAPI_basic Test Details
# Found 30 TestRuns in dev, qa from 2025-10-07 to 2025-11-04 from master branch: 1 unique tests, PASS(x 29) FAIL
Success rate: 96.67%

## Error Table

Date | Details | Env | Runtime
--- | --- | --- | ---
[2025-10-30 00:29](#error-2025-10-30t0029550000) |  | dev | 7.01s

## Timeline
- 2025-10-05: MISSING
- 2025-10-06: MISSING
- 2025-10-07 PASS 8 seconds
- 2025-10-08 PASS 6 seconds
- 2025-10-09 PASS 8 seconds
- 2025-10-10 PASS 8 seconds
- 2025-10-11 PASS 8 seconds
- 2025-10-12 PASS 5 seconds
- 2025-10-13 PASS 6 seconds
- 2025-10-14 PASS 7 seconds
- 2025-10-15 PASS 7 seconds
- 2025-10-16 PASS 9 seconds
- 2025-10-17 PASS 6 seconds
- 2025-10-18 PASS 6 seconds
- 2025-10-19 PASS 7 seconds
- 2025-10-20
  - PASS 7 seconds
  - PASS 5 seconds
- 2025-10-21 PASS 5 seconds
- 2025-10-22
  - PASS 9 seconds
  - PASS 6 seconds
- 2025-10-23 PASS 5 seconds
- 2025-10-24 PASS 6 seconds
- 2025-10-25 PASS 7 seconds
- 2025-10-26 PASS 8 seconds
- 2025-10-27 PASS 5 seconds
- 2025-10-28 PASS 7 seconds
- 2025-10-29 PASS 6 seconds
- 2025-10-30

### Error 2025-10-30T00:29:55+00:00
```
2025-10-30T00:29:55.5760595Z === RUN   TestAccProjectAPI_basic
2025-10-30T00:29:55.5761797Z === CONT  TestAccProjectAPI_basic
2025-10-30T00:29:55.5766331Z     resource_test.go:22: Error running post-test destroy, there may be dangling resources: project (6902b17ce2dc7470847cafa2) still exists
2025-10-30T00:29:55.5766935Z --- FAIL: TestAccProjectAPI_basic (7.06s)
```

- 2025-10-31 PASS 6 seconds
- 2025-11-01: MISSING
- 2025-11-02 PASS 6 seconds
- 2025-11-03 PASS 7 seconds
- 2025-11-04 PASS 8 seconds